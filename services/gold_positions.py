...
async def _open_same_side_today(db: AsyncSession, symbol: str, side: str) -> Optional[GoldPosition]:
...
async def open_from_signal(db: AsyncSession, sig: dict,
    ...
    dup = await _open_same_side_today(db, symbol, side)
    if dup is not None:
        return {"skipped": "duplicate — same side already open today", "id": dup.id}

    tps = [t.get("price") for t in sig.get("targets", [])]
    tps += [None] * (3 - len(tps))
    row = GoldPosition(
        symbol=symbol, side=side, entry=float(entry),
        stop=float(stop), stop_initial=float(stop),
...
async def monitor(db: AsyncSession, price: float,
    ...
    for row in rows:
        ...
        state = {
            ...
            "targets": [t for t in (row.tp1, row.tp2, row.tp3) if t is not None],
...
def _to_dict(r: GoldPosition) -> dict:
    return {
        ...
        "targets": [t for t in (r.tp1, r.tp2, r.tp3) if t is not None],
