...
EXCHANGERATE_HOST_KEY = config("EXCHANGERATE_HOST_KEY", default=None)

BASE_URL_TWELVE = "https://api.twelvedata.com/price"
BASE_URL_FRANKFURTER = "https://api.frankfurter.app/latest"
BASE_URL_EXCHANGERATE = "https://api.exchangerate.host/convert"
...
async def get_fallback_price(symbol: str) -> Optional[float]:
    ...
    if re.fullmatch(r"[A-Z]{6}", pair_clean):
        ...
        if price is not None:
            return price
        price = await _exchangerate_price(base, quote)
        if price is not None:
            return price
...
async def _exchangerate_price(base: str, quote: str) -> Optional[float]:
    """exchangerate.host FX fallback (needs EXCHANGERATE_HOST_KEY)."""
    if not EXCHANGERATE_HOST_KEY:
        return None
    params = {"from": base, "to": quote, "access_key": EXCHANGERATE_HOST_KEY}
    try:
        async with httpx.AsyncClient(timeout=8) as client:
            data = (await client.get(BASE_URL_EXCHANGERATE, params=params)).json()
