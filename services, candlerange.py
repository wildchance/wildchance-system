"""Reference-candle range endpoints — 01:00 / 13:00 UTC continuation vs reversal.

  GET  /candlerange/{symbol}        classify price vs the two anchor candles
  POST /candlerange/scan            classify a watch set, alert on any
                                    continuation/reversal (called after each
                                    anchor hour closes: ~02:00 and ~14:00 UTC)
"""
...
async def _send(text: str) -> bool:
    if not BOT_TOKEN or not CHAT_ID:
        print("[candlerange] telegram not configured — skipping send")
        return False
    ...
    try:
        ...
        print(f"[candlerange] send failed: {e}")
        return False
...
@router.post("/scan")
async def scan(symbols: str = Query(None, description="comma-separated override"),
               force: bool = Query(False)):
    watch = ([s.strip() for s in symbols.split(",") if s.strip()]
             if symbols else DEFAULT_WATCH)
    hits, alerts = [], []
    for sym in watch:
